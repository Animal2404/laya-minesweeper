...
function solve(b, depth) {
  ...
  // ==========================================================================
  // 把所有连通块**联合**枚举，一次性施加全局雷数约束。
  //
  // 为什么不再逐块算"窗口"：逐块枚举时每块只知道自己的雷数分布，
  // 必须靠其它块的 [lo,hi] 反推窗口；这个反推一旦有落差（例如某块枚举
  // 被预算截断），窗口就会算错，概率随之偏掉 —— 实测 7x7 棋盘上与
  // 暴力真值差 44%（真值 100% 是雷的格子被算成 55.6%）。
  //
  // 联合枚举没有这个问题：所有约束 + 全局雷数一起进 DFS，
  // 只有真正自洽的配置才被计入分母。参考实现
  // （github.com/aijobclub/minesweeper-solver）也是这个思路。
  // ==========================================================================
  const allCells = [];
  const allIdx = [];
  for (const g of groups) { allIdx.push(...g.idx); for (const k of g.cells) allCells.push(k); }
  const touched = allCells.length;
  const freeCells = Math.max(0, totalHidden - touched);

  // 自由格承担剩余雷数；块内雷数因此落在 [minesLeft - freeCells, minesLeft]
  // 若块内格子数超过 cap，则退化为"逐块独立枚举 + 全局 DP 近似"
  let joint = null;
  if (allCells.length && allCells.length <= CAP) {
    joint = enumerateGroup(cons, allIdx, allCells, CAP,
                           budgetFor(allCells.length) * 2, deadline,
                           Math.max(0, minesLeft - freeCells), minesLeft);
  }

  if (joint && joint.probs) {
    for (const [k, p] of joint.probs) localProbs.set(k, p);
    enumCells += allCells.length;
    solutions += joint.solutions;
    solved.push({hist: joint.hist, perCell: joint.perCell, cells: allCells});
  } else {
    if (allCells.length > CAP) tooBig++;
    exactAll = false;
    // 降级：逐块独立枚举（不考虑全局雷数），只求给部分格子一个近似值
...
function solverSource() {
  ...
  const parts = [
    ...
    buildConstraints.toString(), key2rc.toString(), components.toString(),
    enumerateGroup.toString(), combineGlobally.toString(),
    binom.toString(), logBinom.toString(), lgamma.toString(), logBinomSafe.toString(),
    splitGroup.toString(), solve.toString(), planMoves.toString(),
    cloneBoard.toString(), applyMove.toString(), countHidden.toString(),
    estimateGlobal.toString(), pot.toString(), hintWorthy.toString(),
  ];
  return parts.filter(p => p && p.length).join(String.fromCharCode(10));
}

function startWorkerRun() {
  try {
    const body = solverSource();
    const workerCode = body + `
      self.onmessage = function(ev){
        ...
        for (let g=0; g<n; g++){
          ...
          if ((g+1) % 5 === 0 || g === n-1) {
            self.postMessage({type:'progress', done:g+1, wins, guessSum, deduceSum, stalled, errors});
          }
        }
        self.postMessage({type:'done', done:n, wins, guessSum, deduceSum, stalled, errors});
      };
    `;
...
function planMoves(b, depth, maxMoves) {
  ...
  for (let round = 0; round < ROUNDS; round++) {
    ...
    // （20% 左右），这时去远处赌一把反而更划算。
    const all = frontier.length ? frontier.slice() : remote.slice();
    if (frontier.length && remote.length) {
      const fp = frontier.reduce((m, x) => Math.min(m, x.p === null ? 1 : x.p), 1);
      const rp = remote[0].p === null ? gp0(sim) : remote[0].p;
      if (rp < fp - 0.20) all.push(...remote);      // 远处明显更安全才纳入
    }
